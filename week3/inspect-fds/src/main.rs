...
fn main() {
    ...
    let target = &args[1];
    let result = ps_utils::get_target(target).expect("Error getting target process!");
    match result {
        Some(process) => {
            process.print();
            let child_processes = ps_utils::get_child_processes(process.pid)
                .expect("Error while getting child processes");
            for process in child_processes {
                process.print();
            }
        }
        None => {
            println!("No matching process \"{}\" found!", target);
            std::process::exit(1);
        }
    }
}
